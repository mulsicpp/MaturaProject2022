...
void gear::Renderer::render_Scene(gear::Scene *scene)
{
    ...
    if (m_Camera != nullptr)
        glUniform2f(glGetUniformLocation(SpritePipeline::get_Instance().m_Shader, "u_Camera_Pos"), round(m_Camera->get_Position()[0]), round(m_Camera->get_Position()[1]));
    else
    ...
    if (m_Camera != nullptr)
        glUniform2f(glGetUniformLocation(TextPipeline::get_Instance().m_Shader, "u_Camera_Pos"), round(m_Camera->get_Position()[0]), round(m_Camera->get_Position()[1]));
    else
...
void gear::Renderer::render_Shape(const gear::Shape *shape, const Vector<float, 4> &color)
{
    ...
    if (m_Camera != nullptr)
        glUniform2f(glGetUniformLocation(ShapePipeline::get_Instance().m_Shader, "u_Camera_Pos"), round(m_Camera->get_Position()[0]), round(m_Camera->get_Position()[1]));
    else
